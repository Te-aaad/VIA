...
<body class="bg-gray-50 font-sans">
                                        ...
                                        @foreach($notifications as $notification)
                                            ...
                                            <div class="py-3 px-1 {{ $notification->read_at ? 'opacity-60' : '' }}">
                                                <div class="flex items-start gap-3">
                                                    <div class="{{ $data->type == 'ready_for_pickup' ? 'text-blue-500' : 'text-red-500' }} mt-1">
                                                        <iconify-icon icon="{{ $data->type == 'ready_for_pickup' ? 'mdi:food-takeout-box' : 'mdi:alert-circle' }}" width="24" height="24"></iconify-icon>
                                                    </div>
                                                    <div class="flex-1">
                                                        ...
                                                        @if($data->type == 'ready_for_pickup')
                                                            <div class="mt-2 bg-yellow-100 rounded-lg p-2">
                                                                <p class="text-xs text-yellow-800 font-semibold">Auto-cancel in: <span class="pickup-countdown" data-order-id="{{ $data->order_id }}" data-updated-at="{{ isset($data->pickup_by) ? $data->pickup_by : $notification->created_at }}">
                                                                    @php
                                                                        // Calculate seconds remaining
                                                                        $startTime = isset($data->pickup_by) 
                                                                            ? \Carbon\Carbon::parse($data->pickup_by)->subSeconds(60) // Subtract 60 seconds from pickup time
                                                                        ...
                                                                        // Debug information
                                                                        \Log::info('Notification timer for Order #'.$data->order_number, [
                                                                            'notification_id' => $notification->id,
                                                                            'start_time' => $startTime->toDateTimeString(),
                                                                            'pickup_by' => isset($data->pickup_by) ? $data->pickup_by : 'not set',
                                                                ...
                                                                <p class="text-xs text-yellow-800">Please pick up soon to avoid cancellation!</p>
                                                            </div>
                                                        @endif
                                                    </div>
                                                </div>
    ...
    <script>
        ...
        // Auto-cancel orders not picked up within 60 seconds
        function checkOrdersForAutoCancellation() {
            console.log('Checking for orders to auto-cancel...');
            $.ajax({
                url: "{{ route('orders.check-ready-for-pickup') }}",
                type: "GET",
                success: function(response) {
                    console.log('Ready orders response:', response);
                    if (response.ready_orders && response.ready_orders.length > 0) {
                        console.log(`Found ${response.ready_orders.length} orders in 'ready' status`);
                        response.ready_orders.forEach(order => {
                            const readyTime = new Date(order.updated_at);
                            const currentTime = new Date();
                            const timeDifference = (currentTime - readyTime) / 1000; // in seconds
                            
                            console.log(`Order #${order.order_number} (ID: ${order.order_id}) has been ready for ${Math.floor(timeDifference)} seconds`);
                            
                            if (timeDifference >= 60) {
                                console.log(`Auto-cancelling order #${order.order_number} (ID: ${order.order_id})`);
                                // Cancel the order automatically if not picked up within 60 seconds
                                $.ajax({
                                    ...
                                    success: function(cancelResponse) {
                                        ...
                                        if (cancelResponse.success) {
                                            console.log(`Order #${order.order_number} was successfully auto-cancelled`);
                                            showMessage(`Order #${order.order_number} was automatically cancelled because it wasn't picked up within 60 seconds.`, 'warning');
                                            ...
                                            console.error(`Failed to auto-cancel order #${order.order_number}:`, cancelResponse.message);
                                        }
                                    },
                                    error: function(xhr, status, error) {
                                        console.error(`Error auto-cancelling order #${order.order_number}:`, error);
                        ...
                        console.log('No orders in ready status found');
                    }
                },
                error: function(xhr, status, error) {
        ...
        $(document).ready(function() {
            ...
            setInterval(checkOrdersForAutoCancellation, 5000); // Check every 5 seconds
            loadCart();
            checkPendingOrders();
            ...
            console.log('Auto-cancellation check is running every 5 seconds');
        });
            ...
            countdownElements.forEach(element => {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (response.should_cancel) {
                            ...
                            $.ajax({
                                ...
                                success: function(response) {
                                    console.log('Auto-cancel response from notification:', response);
                                },
                                error: function(error) {
                                    console.error('Error auto-cancelling order from notification:', error);
                                }
                            });
                            
                            return; // Skip setting up timer
                        }
                        ...
                        const timerId = setInterval(() => {
                            try {
                                ...
                                // Debug timer values occasionally
                                if (remainingSeconds % 10 === 0) {
                                    console.log(`Timer update for order ${orderId}: ${remainingSeconds}s remaining`)
                                }
                    ...
                    error: function(xhr, status, error) {
                        ...
                        // Continue with client-side time calculation as fallback
